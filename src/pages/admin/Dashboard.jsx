      ...
      <div className="m-5">
        <div className="flex flex-wrap gap-3">
          <div className="flex items-center gap-2 bg-gray  bg-opacity-10 p-4 min-w-72 rounded border-2 border-primary cursor-pointer hover:scale-105 transition-all">
            <img className="w-14" src={assets.consultant_icon} alt="" />
            <div>
              <p className="text-xl font-semibold text-gray-200">
                {dashData.consultants}
              </p>
              <p className="text-gray-100">Consultants</p>
            </div>
          </div>
        ...
        <div className="bg-gray bg-opacity-10 ">
          ...
          <div className="border border-primary">
            {dashData.latestAppointments.map((item, index) => (
              <div className="flex items-center px-6 py-3 gap-3 border-b border-primary hover:bg-primary hover:bg-opacity-10" key={index}>
                <img className="rounded-full w-10" src={item.consData.image} alt="" />
                <div className="flex-1 text-sm">
                  <p className="text-gray-200 font-medium">{item.consData.name}</p>
                  <p className="text-gray-100">{ slotDateFormat(item.slotDate)} </p>
                </div>
                {item.cancelled
